...
export function ProductionProcessesPage({ currentUser }: { currentUser: User }) {
  return <ProductionPageLayout title="工序定义" description="维护生产节点，并明确每道工序可执行的岗位角色和员工账号。"><ProcessesPanel currentUser={currentUser} /></ProductionPageLayout>;
}

export function ProductionRoutesPage({ currentUser }: { currentUser: User }) {
  return <ProductionPageLayout title="工艺路线" description="按产品配置工序顺序、执行部门、质量关卡和半成品/成品输出仓。"><RoutesPanel currentUser={currentUser} /></ProductionPageLayout>;
}
...
function ProcessesPanel({ currentUser }: { currentUser: User }) {
...
function WorkOrdersPanel({ currentUser }: { currentUser: User }) {
  ...
  return (
    <section className="panel">
      ...
      <div className="table-wrap"><table className="production-table"><thead><tr><th>工单</th><th>产品明细</th><th>路线 / 部门</th><th>总计划数量</th><th>最终合格 / 报废</th><th>状态</th><th>负责人</th><th className="action-cell">操作</th></tr></thead><tbody>
        {items.map((item) => <tr key={item.id}>
          ...
          <td><strong>{item.itemSummaries.length === 1 ? item.itemSummaries[0].routeName : "按产品明细路线执行"}</strong><small>{item.departmentName}</small></td>
...
function ProcessForm({ item, onClose, onSaved }: { item: ProductionProcess | null; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({ code: item?.code ?? "", name: item?.name ?? "", processType: item?.processType ?? "manufacturing", sortOrder: String(item?.sortOrder ?? 0), description: item?.description ?? "", status: item?.status ?? "active", roleIds: [] as number[], userIds: [] as number[] });
  useEffect(() => { void Promise.all([request<ProductionAuthorizationOptions>("/production/authorization-options"), item ? request<{ authorizations: { roleIds: number[]; userIds: number[] } }>(`/production/processes/${item.id}`) : Promise.resolve(null)]).then(([loadedOptions, detail]) => { setOptions(loadedOptions); if (detail) setForm((current) => ({ ...current, roleIds: detail.authorizations.roleIds, userIds: detail.authorizations.userIds })); }); }, [item]);
  const toggle = (key: "roleIds" | "userIds", id: number) => setForm((current) => ({ ...current, [key]: current[key].includes(id) ? current[key].filter((value) => value !== id) : [...current[key], id] }));
  if (!options) return <SimpleModal title={item ? "编辑工序" : "新建工序"} onClose={onClose}><div className="modal-form">正在加载工序授权配置...</div></SimpleModal>;
  return <EntityModal title={item ? "编辑工序" : "新建工序"} onClose={onClose} onSubmit={async () => { const result = await request<{ item: { id: number } }>(item ? `/production/processes/${item.id}` : "/production/processes", { method: item ? "PUT" : "POST", body: JSON.stringify({ code: form.code, name: form.name, processType: form.processType, sortOrder: Number(form.sortOrder), description: form.description, status: form.status }) }); await request(`/production/processes/${result.item.id}/authorizations`, { method: "PUT", body: JSON.stringify({ roleIds: form.roleIds, userIds: form.userIds }) }); }} onSaved={onSaved}><div className="form-grid"><label>工序名称<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>工序编码<input value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value })} placeholder="留空自动生成" /></label><label>工序类型<select value={form.processType} onChange={(event) => setForm({ ...form, processType: event.target.value as ProcessType })}>{Object.entries(processTypeLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label><label>排序<input type="number" value={form.sortOrder} onChange={(event) => setForm({ ...form, sortOrder: event.target.value })} /></label><label>状态<select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as "active" | "inactive" })}><option value="active">启用</option><option value="inactive">停用</option></select></label><label className="full-span">工序说明<textarea rows={2} value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label></div><div className="role-picker"><div className="field-label">可执行岗位角色</div><div className="role-options">{options.roles.map((role) => <button type="button" className={`role-option ${form.roleIds.includes(role.id) ? "selected" : ""}`} key={role.id} onClick={() => toggle("roleIds", role.id)}><span>{form.roleIds.includes(role.id) ? <Check size={14} /> : <span className="empty-check" />}</span>{role.name}</button>)}</div></div><div className="role-picker"><div className="field-label">额外指定员工</div><div className="role-options">{options.users.map((user) => <button type="button" className={`role-option ${form.userIds.includes(user.id) ? "selected" : ""}`} key={user.id} onClick={() => toggle("userIds", user.id)}><span>{form.userIds.includes(user.id) ? <Check size={14} /> : <span className="empty-check" />}</span>{user.displayName} · {user.position || user.departmentName || "员工"}</button>)}</div></div></EntityModal>;
}
...
function RouteForm({ item, onClose, onSaved }: { item: ProductionRoute | null; onClose: () => void; onSaved: () => void }) {
  ...
  const blankStep = () => ({ processId: "", defaultDepartmentId: "", outputTarget: "next_process" as OutputTarget, outputItemId: "", outputWarehouseId: "", qualityGate: false, description: "" });
  ...
  useEffect(() => { const load = async () => { const [processResult, itemResult, optionResult] = await Promise.all([request<{ items: ProductionProcess[] }>("/production/processes"), request<{ items: ProductItem[] }>("/inventory/items"), request<ProductionRouteOptions>("/production/route-options")]); setProcesses(processResult.items.filter((entry) => entry.status === "active")); setProducts(itemResult.items.filter((entry) => entry.status === "active")); setOptions(optionResult); if (item) { const detail = await request<{ steps: ProductionRouteStep[] }>(`/production/routes/${item.id}`); setForm((current) => ({ ...current, steps: detail.steps.map((step) => ({ processId: String(step.processId), defaultDepartmentId: step.defaultDepartmentId?.toString() ?? "", outputTarget: step.outputTarget, outputItemId: step.outputItemId?.toString() ?? "", outputWarehouseId: step.outputWarehouseId?.toString() ?? "", qualityGate: Boolean(step.qualityGate), description: step.description })) })); } setLoading(false); }; void load(); }, [item]);
  const addStep = () => setForm((current) => ({ ...current, steps: [...current.steps, blankStep()] }));
  const changeStep = (index: number, key: "processId" | "defaultDepartmentId" | "outputTarget" | "outputItemId" | "outputWarehouseId" | "qualityGate" | "description", value: string | boolean) => setForm((current) => ({ ...current, steps: current.steps.map((step, stepIndex) => stepIndex === index ? { ...step, [key]: value } : step) }));
...
function WorkOrderForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => { void Promise.all([request<{ items: ProductItem[] }>("/inventory/items"), request<{ items: ProductionRoute[] }>("/production/routes"), request<{ items: Department[] }>("/departments"), request<{ items: Operator[] }>("/production/operators")]).then(([productResult, routeResult, departmentResult, operatorResult]) => { setProducts(productResult.items.filter((entry) => entry.status === "active")); setRoutes(routeResult.items.filter((entry) => entry.status === "active")); setDepartments(departmentResult.items.filter((entry) => entry.status === "active")); setOperators(operatorResult.items); }); }, []);
  return <EntityModal title="新建生产工单" onClose={onClose} onSubmit={async () => { await request("/production/work-orders", { method: "POST", body: JSON.stringify({ items: form.lines.map((line) => ({ productItemId: Number(line.productItemId), routeId: Number(line.routeId), plannedQuantity: Number(line.plannedQuantity), remark: line.remark })), departmentId: Number(form.departmentId), managerUserId: form.managerUserId ? Number(form.managerUserId) : null, priority: form.priority as WorkOrderPriority, plannedStartDate: form.plannedStartDate, plannedEndDate: form.plannedEndDate, remark: form.remark }) }); }} onSaved={onSaved}>
...
function AssignTaskModal({ task, onClose, onSaved }: { task: ProductionTask; onClose: () => void; onSaved: () => void }) {
  const [departments, setDepartments] = useState<Department[]>([]);
  const [operators, setOperators] = useState<Operator[]>([]);
  const [form, setForm] = useState({ assignedDepartmentId: task.assignedDepartmentId?.toString() ?? "", assignedUserId: task.assignedUserId?.toString() ?? "", remark: task.remark ?? "" });
  useEffect(() => { void request<{ items: Department[] }>("/departments").then((result) => setDepartments(result.items.filter((entry) => entry.status === "active"))); }, []);
  useEffect(() => { if (!form.assignedDepartmentId) { setOperators([]); return; } void request<{ items: Operator[] }>(`/production/operators?processId=${task.processId}&departmentId=${form.assignedDepartmentId}`).then((result) => setOperators(result.items)); }, [form.assignedDepartmentId, task.processId]);
  return <EntityModal title={`派工 · ${task.taskNo}`} onClose={onClose} onSubmit={async () => { await request(`/production/tasks/${task.id}/assign`, { method: "POST", body: JSON.stringify({ assignedDepartmentId: Number(form.assignedDepartmentId), assignedUserId: Number(form.assignedUserId), remark: form.remark }) }); }} onSaved={onSaved}><div className="form-note">系统仅列出拥有当前工序执行授权的启用员工。</div><div className="form-grid"><label>派工部门<select value={form.assignedDepartmentId} disabled={Boolean(task.assignedDepartmentId)} onChange={(event) => setForm({ ...form, assignedDepartmentId: event.target.value, assignedUserId: "" })}><option value="">请选择部门</option>{departments.map((department) => <option value={department.id} key={department.id}>{department.name}</option>)}</select></label><label>执行员工<select value={form.assignedUserId} onChange={(event) => setForm({ ...form, assignedUserId: event.target.value })}><option value="">请选择执行员工</option>{operators.map((operator) => <option value={operator.id} key={operator.id}>{operator.displayName} · {operator.employeeNo}</option>)}</select></label><label className="full-span">派工备注<textarea rows={3} value={form.remark} onChange={(event) => setForm({ ...form, remark: event.target.value })} /></label></div></EntityModal>;
}
...
export function downloadProductionTask(detail: ProductionTaskDetail) {
  ...
  const rows = [
    ...
    ["执行部门", task.assignedDepartmentName || ""],
